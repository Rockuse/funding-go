...
type Repository interface {
	...
	GetByCampaign(campaignId int, userId int) ([]Transaction, error)
}
...
func (r *repository) GetByCampaign(campaignId int, userId int) ([]Transaction, error) {
	var transaction []Transaction
	err := r.db.Preload("Campaign.User").
		Where("campaign_id=?", campaignId).
		Find(&transaction, "user_id=?", userId).Error
	if err != nil {
		return transaction, err
	}
	return transaction, nil
